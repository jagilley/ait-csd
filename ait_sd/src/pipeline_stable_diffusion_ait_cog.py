...
class StableDiffusionAITSlimPipeline(DiffusionPipeline):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion. No longer needs SD vae, unet, or text encoder.
    ...
    def init_modules(self, workdir="./weights"):
        """Hack to workaround diffusers pipeline init"""
        with open(os.path.join(workdir, "config.json"), "r") as f:
            config = json.load(f)

        self.unet_in_channels = int(config["in_channels"])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        seqlen = self.tokenizer.model_max_length
        ...
        print(f'input_shape: {text_input.input_ids.shape}')
        text_embeddings = self.clip_inference(text_input.input_ids.to(self.device), seqlen=seqlen)
        ...
        if do_classifier_free_guidance:
            ...
            uncond_embeddings = self.clip_inference(
                uncond_input.input_ids.to(self.device), seqlen=seqlen
            )
